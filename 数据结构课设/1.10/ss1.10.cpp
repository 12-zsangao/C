...
void add_bus_route() {
    ...
    if (new_route.station_count <= 0 || new_route.station_count > MAX_STATIONS) {
        printf("站点数量无效。\n");
        return;
    }
    ...
    for (int i = 0; i < new_route.station_count; i++) {
        ...
        stations[id-1].is_bus_station = 1;
    }
...
void modify_bus_route() {
    ...
    for (int i = 0; i < route_count; i++) {
        if (bus_routes[i].id == route_id) {
            ...
            if (bus_routes[i].station_count <= 0 || bus_routes[i].station_count > MAX_STATIONS) {
                printf("站点数量无效。\n");
                return;
            }
            ...
            save_data();
            printf("公交线路修改成功：线路ID=%d，站点数=%d\n站点：", bus_routes[i].id, bus_routes[i].station_count);
            for (int j = 0; j < bus_routes[i].station_count; j++) {
...
void delete_station_from_route() {
    ...
    for (int i = 0; i < route_count; i++) {
        if (bus_routes[i].id == route_id) {
            ...
            if (new_count == bus_routes[i].station_count) {
                printf("线路中未找到该站点。\n");
                return;
            }
            ...
            // 更新站点是否为公交站点
            int is_bus_station = 0;
            for (int r = 0; r < route_count; r++) {
                for (int s = 0; s < bus_routes[r].station_count; s++) {
                    if (bus_routes[r].stations[s] == station_id) {
                        is_bus_station = 1;
                        break;
                    }
                }
                if (is_bus_station) break;
            }
            stations[station_id-1].is_bus_station = is_bus_station;
...
Path dijkstra(int start_id, int end_id, int mode) {
    ...
    float total_dist[MAX_STATIONS]; // 用于最少换乘模式比较距离
