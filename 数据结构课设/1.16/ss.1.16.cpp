...
void delete_station_from_route() {
    ...
    for (int j = 0; j < bus_routes[route_idx].station_count; j++) {
        if (bus_routes[route_idx].stations[j] != station_id) {
            new_stations[new_count] = bus_routes[route_idx].stations[j];
            if (j < bus_routes[route_idx].station_count - 1 && new_count < bus_routes[route_idx].station_count - 1) {
                new_distances[new_count] = bus_routes[route_idx].distances[j];
            }
            new_count++;
        }
    }
    
    if (new_count == bus_routes[route_idx].station_count) {
        printf("线路中未找到该站点。\n");
        return;
    }
