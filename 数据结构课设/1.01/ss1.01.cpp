...
void init_default_data() {
    ...
    walk_edge_count = 46; // 更新步行路径总数，包含双向路径
...
Path dijkstra(int start_id, int end_id, int mode) { // mode: 0 for min time, 1 for min transfers, 2 for min distance
    ...
    for (int i = 0; i < station_count; i++) {
        ...
        for (int j = 0; j < station_count; j++) {
            if (!visited[j] && (mode == 1 ? transfers[j] < transfers[min_idx] || (transfers[j] == transfers[min_idx] && dist[j] < min_dist) : dist[j] < min_dist)) {
                min_dist = dist[j];
                min_idx = j;
            }
        }
        
        if (min_idx == -1 || min_idx == end_id-1) break;
        visited[min_idx] = 1;
                    ...
                    if (j != route_start_idx) {
                        ...
                        if (!visited[next_station]) {
                            ...
                            if (mode == 1 ? new_transfers < transfers[next_station] || 
                                            (new_transfers == transfers[next_station] && new_dist < dist[next_station]) :
    ...
    if (dist[end_id-1] != 99999.0) {
        ...
        if (mode == 0) {
            result.time = result.distance / (strcmp(result.modes[0], "步行") == 0 ? WALK_SPEED : BUS_SPEED);
        }
    }
