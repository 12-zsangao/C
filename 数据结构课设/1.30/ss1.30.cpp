...
void load_data() {
    ...
    walk_edge_count = (walk_edge_count > MAX_EDGES) ? MAX_EDGES : walk_edge_count; // 限制最大值
    for (int i = 0; i < walk_edge_count; i++) {
        int from, to;
        float distance;
...
void update_walk_edges() {
    walk_edge_count = 0; // 重置步行边计数
    
    // 添加原有的步行边
    int idx = 0;
    walk_edges[idx++] = (WalkEdge){1, 2, 0.25};   walk_edges[idx++] = (WalkEdge){2, 1, 0.25};    // 五棵松—教务处
    walk_edges[idx++] = (WalkEdge){1, 11, 1.5};   walk_edges[idx++] = (WalkEdge){11, 1, 1.5};    // 五棵松—宿舍C区
    walk_edges[idx++] = (WalkEdge){1, 7, 1.0};    walk_edges[idx++] = (WalkEdge){7, 1, 1.0};     // 五棵松—三岔口
    walk_edges[idx++] = (WalkEdge){1, 6, 1.0};    walk_edges[idx++] = (WalkEdge){6, 1, 1.0};     // 五棵松—大礼堂
    walk_edges[idx++] = (WalkEdge){2, 3, 0.25};   walk_edges[idx++] = (WalkEdge){3, 2, 0.25};    // 教务处—图书馆
    walk_edges[idx++] = (WalkEdge){4, 5, 0.5};    walk_edges[idx++] = (WalkEdge){5, 4, 0.5};     // 教职工活动中心—职工医院
    walk_edges[idx++] = (WalkEdge){7, 12, 0.5};   walk_edges[idx++] = (WalkEdge){12, 7, 0.5};    // 三岔口—宿舍B区
    walk_edges[idx++] = (WalkEdge){11, 12, 0.25}; walk_edges[idx++] = (WalkEdge){12, 11, 0.25};  // 宿舍C区—宿舍B区
    walk_edges[idx++] = (WalkEdge){12, 6, 1.0};   walk_edges[idx++] = (WalkEdge){6, 12, 1.0};    // 宿舍B区—大礼堂
    walk_edges[idx++] = (WalkEdge){12, 13, 0.25}; walk_edges[idx++] = (WalkEdge){13, 12, 0.25};  // 宿舍B区—宿舍D区
    walk_edges[idx++] = (WalkEdge){6, 5, 1.0};    walk_edges[idx++] = (WalkEdge){5, 6, 1.0};     // 大礼堂—职工医院
    walk_edges[idx++] = (WalkEdge){5, 14, 1.0};   walk_edges[idx++] = (WalkEdge){14, 5, 1.0};    // 职工医院—行政二号楼
    walk_edges[idx++] = (WalkEdge){6, 14, 1.0};   walk_edges[idx++] = (WalkEdge){14, 6, 1.0};    // 大礼堂—行政二号楼
    walk_edges[idx++] = (WalkEdge){13, 14, 1.0};  walk_edges[idx++] = (WalkEdge){14, 13, 1.0};   // 宿舍D区—行政二号楼
    walk_edges[idx++] = (WalkEdge){14, 16, 1.0};  walk_edges[idx++] = (WalkEdge){16, 14, 1.0};   // 行政二号楼—六食堂
    walk_edges[idx++] = (WalkEdge){13, 16, 0.5};  walk_edges[idx++] = (WalkEdge){16, 13, 0.5};   // 宿舍D区—六食堂
    walk_edges[idx++] = (WalkEdge){16, 15, 0.5};  walk_edges[idx++] = (WalkEdge){15, 16, 0.5};   // 六食堂—京江楼
    walk_edges[idx++] = (WalkEdge){16, 10, 0.5};  walk_edges[idx++] = (WalkEdge){10, 16, 0.5};   // 六食堂—宿舍G区
    walk_edges[idx++] = (WalkEdge){10, 9, 1.0};   walk_edges[idx++] = (WalkEdge){9, 10, 1.0};    // 宿舍G区—耒耜大楼
    walk_edges[idx++] = (WalkEdge){9, 15, 0.5};   walk_edges[idx++] = (WalkEdge){15, 9, 0.5};    // 耒耜大楼—京江楼
    walk_edges[idx++] = (WalkEdge){9, 8, 0.5};    walk_edges[idx++] = (WalkEdge){8, 9, 0.5};     // 耒耜大楼—研究生院
    walk_edges[idx++] = (WalkEdge){8, 3, 0.5};    walk_edges[idx++] = (WalkEdge){3, 8, 0.5};     // 研究生院—图书馆
    walk_edges[idx++] = (WalkEdge){9, 1, 1.5};    walk_edges[idx++] = (WalkEdge){1, 9, 1.5};     // 耒耜大楼—五棵松
    walk_edges[idx++] = (WalkEdge){9, 3, 1.0};    walk_edges[idx++] = (WalkEdge){3, 9, 1.0};     // 耒耜大楼—图书馆
    
    walk_edge_count = idx;
    
    // 为公交线路上的相邻站点添加步行边（如果不存在）
...
void add_new_station() {
    ...
    new_station.is_bus_station = 1; // 默认公交站点
...
void add_bus_route() {
    ...
    update_walk_edges(); // 新增公交线路后更新步行边
...
void modify_bus_route() {
    ...
    switch (sub_choice) {
        ...
        case 2: {
            ...
            update_bus_station_status();
            update_walk_edges(); // 修改公交线路后更新步行边
...
Path dijkstra(int start_id, int end_id, int mode) {
    ...
    float total_dist[MAX_STATIONS]; // 总距离（用于最少换乘模式比较）
...
void admin_menu() {
    ...
    while (1) {
        ...
        switch (sub_choice) {
            ...
            case 7: display_all_routes(); break;  // 新增功能
            case 8: return;
            case 9:
                printf("感谢使用本系统。\n");
                exit(0);
